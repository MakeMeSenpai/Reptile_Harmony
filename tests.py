...
import unittest 
from app import app, index, shop, cart, other
...
class tester_Liz(unittest.TestCase):
    ...
    def tests_shop(self):
            ...
            self.assertEqual(shop(), render_template("shop.html"))

    #tests our cart page
    def tests_cart(self):
            result = self.client.get('/cart')
            self.assertEqual('/cart', result)
            self.assertEqual(cart(), render_template("cart.html"))

    #tests our other page
    def tests_other(self):
            result = self.client.get('/other')
            self.assertEqual('/other', result)
            self.assertEqual(other(), render_template("other.html"))
